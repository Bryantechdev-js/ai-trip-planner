...
const testLygosPayment = async () => {
  const testPayload = {
    ...
    success_url: "http://localhost:3000/api/payment/callback?status=success&orderId=test_123&userId=test_user&planId=pro",
...
const testPhoneValidation = () => {
  ...
  testNumbers.forEach(number => {
    ...
    if (isValid) {
      ...
      else if (['69', '77', '78', '79'].includes(prefix) || phoneWithoutCountryCode.startsWith('7')) {
        provider = 'Orange'
      }

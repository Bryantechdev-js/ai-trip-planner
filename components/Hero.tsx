    ...
    <section className="mt-12 sm:mt-24 flex items-center justify-center px-4">
      <div className="max-w-full text-center space-y-4 sm:space-y-6 animate-fadeIn">
        <h1 className="text-2xl sm:text-3xl md:text-5xl font-bold leading-tight">
          {!user ? 'Hey' : `Welcome back ${user?.firstName}`}, I'm your personal{' '}
          <span className="text-primary">Trip Planner</span>
        </h1>
        <p className="text-base sm:text-lg max-w-2xl mx-auto">
          Tell me what you want, and I'll handle the rest: Flights, Hotels, trip planner
        </p>
        <div className="border rounded-2xl p-3 sm:p-4 relative max-w-2xl mx-auto transform hover:scale-105 transition-transform">
          <Textarea
            placeholder="Create your trip today - Tell me where you want to go!"
            className="w-full h-20 sm:h-28 border-transparent focus-visible:ring-0 shadow-none resize-none text-sm sm:text-base"
          />
          <Button
            size={'icon'}
            className="absolute bottom-3 sm:bottom-4 right-3 sm:right-4 bg-primary text-white hover:bg-primary/80 transform hover:scale-110 transition-transform"

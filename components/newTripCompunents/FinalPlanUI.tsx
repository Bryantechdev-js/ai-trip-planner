...
import { CheckCircle, MapPin, Calendar, Users, DollarSign, Plane, Hotel, Save, Share2, Star, Clock, Camera, Navigation, Sparkles, Heart, Download, ExternalLink } from 'lucide-react'
...
const FinalPlanUI = () => {
  ...
  const fetchDestinationImage = async () => {
    ...
    try {
      ...
      if (result.data && result.data.length > 0) {
        ...
        const bestImage = result.data.find((img: any) => img.source === 'Unsplash') || result.data[0]
        setDestinationImage(bestImage.url)
      } else {
        setDestinationImage(`https://source.unsplash.com/1200x800/?${encodeURIComponent(tripData.destination)},landmark,travel`)
      }
    } catch (error) {
  ...
  return (
    <div className={`w-full max-w-6xl bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-xl transition-all duration-1000 ${showAnimation ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-bold text-white mb-1">
                  {tripData.destination ? `Trip to ${tripData.destination}` : 'Your Amazing Journey'}
                </h1>
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-100">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="bg-white rounded-xl p-4 shadow-sm border border-blue-100">
                  ...
                  <span className="text-lg font-bold text-gray-800">{tripData.destination || 'Not specified'}</span>
                </div>
                ...
                <div className="bg-white rounded-xl p-4 shadow-sm border border-blue-100">
                  ...
                  <span className="text-lg font-bold text-gray-800">{tripData.groupSize || 'Not specified'}</span>
                </div>
                ...
                <div className="bg-gradient-to-br from-yellow-50 to-orange-50 rounded-xl p-4 shadow-sm border border-yellow-200">
                  ...
                  <span className="text-lg font-bold text-yellow-700">{tripData.budget || 'Not specified'}</span>
                </div>
              </div>
                ...
                <div className="mt-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {tripData.interests.map((interest, idx) => (
                      <span key={idx} className="px-3 py-1 bg-gradient-to-r from-purple-100 to-pink-100 text-purple-700 text-sm rounded-full border border-purple-200 font-medium">
                        {interest}
                      </span>
                    ))}
                  </div>
            ...
            <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-6 border border-purple-100">
              ...
              <div className="space-y-3 max-h-80 overflow-y-auto">
                {[
                  { day: 1, activity: `Arrival in ${tripData.destination}`, icon: "✈️", time: "Morning" },
          ...
          {savedTripId && (
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-2xl p-6 animate-fade-in">
              ...
              <p className="text-sm text-green-700">
                Your amazing trip to {tripData.destination} has been saved and {isPublic ? 'is now visible to other travelers for inspiration' : 'is kept private in your collection'}.
              </p>
            </div>
          )}
        </div>

        ...
        {budgetOptions.map(option => (
          <div
            ...
            className={`p-4 sm:p-5 border-2 rounded-xl cursor-pointer transition-all duration-200 ${
              selectedBudget === option.id ? 'border-primary bg-primary/5 shadow-md' : option.color
            }`}
          >
            <div className="flex items-start gap-4">
              ...
              <div
                className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                  selectedBudget === option.id ? 'border-primary bg-primary' : 'border-gray-300'
                }`}
              >
                {selectedBudget === option.id && (
      ...
      {selectedBudget && (
        <div className="mt-6 p-4 bg-primary/5 rounded-lg border border-primary/20">
          <p className="text-sm text-primary font-medium">
            ✓ Great choice! I'll find options within your{' '}
            {budgetOptions.find(opt => opt.id === selectedBudget)?.label.toLowerCase()} budget
            range.
          </p>
        </div>
      )}

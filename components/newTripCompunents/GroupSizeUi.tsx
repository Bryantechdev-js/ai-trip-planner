        ...
        {groupOptions.map(option => (
          <div
            ...
            className={`p-4 sm:p-5 border-2 rounded-xl cursor-pointer transition-all duration-200 ${
              selectedGroup === option.id ? 'border-primary bg-primary/5 shadow-md' : option.color
            }`}
          >
            <div className="flex items-center gap-4">
              ...
              <div
                className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                  selectedGroup === option.id ? 'border-primary bg-primary' : 'border-gray-300'
                }`}
              >
                {selectedGroup === option.id && (
      ...
      {selectedGroup && (
        <div className="mt-6 p-4 bg-primary/5 rounded-lg border border-primary/20">
          <p className="text-sm text-primary font-medium">
            ✓ Perfect! I'll tailor recommendations for{' '}
            {groupOptions.find(opt => opt.id === selectedGroup)?.label.toLowerCase()} travel.
          </p>
        </div>
      )}
    </div>

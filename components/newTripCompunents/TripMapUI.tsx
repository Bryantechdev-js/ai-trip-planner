...
import { calculateDistance, estimateTravelTime, formatDuration, getRouteType, type Coordinates } from '@/lib/mapUtils'
...
const TripMapUI = ({ source, destination, onContinue }: TripMapUIProps) => {
      ...
      try {
        ...
        if (sourceCoords && destCoords) {
          ...
          const sourceMarker = L.marker(sourceCoords, {
            title: source
          }).addTo(map).bindPopup(`<b>Start:</b> ${source}`);
          ...
          // Draw route line
          const routeLine = L.polyline([sourceCoords, destCoords], {
            color: '#3b82f6',
            weight: 4,
            opacity: 0.8,
            dashArray: '10, 5'
  ...
  const toggleViewMode = useCallback(() => {
    ...
    const tileUrl = newMode === 'satellite' 
  ...
  return (
    <div className="w-full max-w-4xl bg-white rounded-xl border border-gray-200 p-3 sm:p-6 animate-slideInUp">
      <div className="flex items-center gap-3 mb-4 sm:mb-6">
        ...
        <div>
          <h3 className="text-base sm:text-lg lg:text-xl font-semibold text-gray-800">Live Route Map</h3>
          <p className="text-xs sm:text-sm text-gray-600">Interactive route from {source} to {destination}</p>
        </div>
      </div>

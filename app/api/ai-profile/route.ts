...
export async function PUT(req: NextRequest) {
  try {
    ...
    const updatedProfile = user.aiProfile || {
      travelPreferences: {
        budgetRange: 'moderate',
        preferredDestinations: [],
        travelStyle: 'balanced',
    ...
    switch (action) {
      case 'add_destination':
        if (
          data.destination &&
          !updatedProfile.travelPreferences.preferredDestinations.includes(data.destination)
        ) {
      ...
      case 'add_activity':
        if (
          data.activity &&
          !updatedProfile.learningData.favoriteActivities.includes(data.activity)
        ) {
      ...
      case 'complete_trip':
        ...
        if (data.duration) {
          ...
          updatedProfile.learningData.avgTripDuration = Math.round(
            (currentAvg * (tripCount - 1) + data.duration) / tripCount
          )
        }
        break
...
function extractLearningInsights(tripData: any, currentProfile: any) {
  ...
  if (tripData.interests && Array.isArray(tripData.interests)) {
    ...
    const newActivities = tripData.interests.filter(
      (activity: string) => !currentActivities.includes(activity)
    )

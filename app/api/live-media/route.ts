...
async function fetchPixabayMedia(destination: string, limit: number) {
  ...
  try {
    const [imageResponse, videoResponse] = await Promise.allSettled([
      fetch(
        `https://pixabay.com/api/?key=${PIXABAY_API_KEY}&q=${encodeURIComponent(destination)}&image_type=photo&per_page=${Math.ceil(limit / 2)}&safesearch=true`
      ),
      fetch(
        `https://pixabay.com/api/videos/?key=${PIXABAY_API_KEY}&q=${encodeURIComponent(destination)}&per_page=${Math.ceil(limit / 2)}&safesearch=true`
      ),
    ])

...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      savings: {
        distanceSaved:
          Math.round((calculateTotalDistance(formattedLocations) - totalDistance) * 100) / 100,
        timeSaved: Math.round(
          estimateTravelTime(calculateTotalDistance(formattedLocations), travelMode) - estimatedTime
        ),
      },
    })
  } catch (error) {

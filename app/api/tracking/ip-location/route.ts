...
export async function GET(req: NextRequest) {
  try {
    ...
    const response = await fetch(`http://ip-api.com/json/${clientIP}?fields=status,message,country,countryCode,region,regionName,city,zip,lat,lon,timezone,isp,org,as,query`);
...
function getFallbackLocationByTimezone(timezone: string) {
  const timezoneMap: { [key: string]: { latitude: number; longitude: number; city: string; country: string } } = {
    ...
    'Africa/Yaounde': { latitude: 3.8480, longitude: 11.5021, city: 'Yaounde', country: 'Cameroon' }
  };

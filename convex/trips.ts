...
export const createTrip = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const coverImage = args.coverImage || `https://source.unsplash.com/1200x800/?${encodeURIComponent(args.destination)},travel,landmark`;
...
export const getTrendingDestinations = query({
  handler: async (ctx) => {
    const trips = await ctx.db.query("TripTable")
